...
#ifndef BENCHMARK_FLOAT_SCOREONLY_CUH
#define BENCHMARK_FLOAT_SCOREONLY_CUH
...
BenchmarkData benchmark_32(
    ...
    if constexpr(ApproachAndType::datatype != Datatype::Int && ApproachAndType::datatype != Datatype::Float){
    ...
    using ScalarScoreType = ScoreType;

    libmarv::GaplessAlignment_scalarscore<21, ApproachAndType, blocksize, groupsize, numItems, isSingleTileBenchmark, !isSingleTileBenchmark> aligner;
    ...
    for(int i = 0; i < numAlignments; i++){
        ...
        // h_subjectBeginOffsets[i] = size_t(53) * subjectLengthPadded;
    }
    // std::copy(
    //     stuff.h_encodedSubjects.begin() + size_t(53) * subjectLengthPadded, 
    //     stuff.h_encodedSubjects.begin() + size_t(54) * subjectLengthPadded,
    //     stuff.h_encodedSubjects.begin()
    // );
    ...
    if(checkResults){    
        ...
        for(int i = 0; i < numAlignments; i++){
            ...
            // std::cout << "cpu h_subjectBeginOffsets = " << h_subjectBeginOffsets[i] << "\n";
            // for(int k = 0; k < subjectLength; k++){
            //     std::cout << int((stuff.h_encodedSubjects.data() + h_subjectBeginOffsets[i])[k]) << " ";
            // }
            // std::cout << "\n";
        }
...
struct ForEachCombination_call_singleconfigFunction{
    ...
    BenchmarkData helper2(
        ...
        if constexpr(ApproachAndType::datatype == Datatype::Float){
            const int queryLength = groupsize * numItems * tiles;
            if(queryLength > 2048){
                return makeNotExecutedBenchmarkData<blocksize, groupsize, numItems, 0, ApproachAndType>();
            }
            return benchmark_32<blocksize, groupsize, numItems, ApproachAndType, SleepDuration, singleTile>(
