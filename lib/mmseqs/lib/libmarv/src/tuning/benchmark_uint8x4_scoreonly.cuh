...
#ifndef BENCHMARK_UINT8X4_SCOREONLY_CUH
#define BENCHMARK_UINT8X4_SCOREONLY_CUH
...
>
BenchmarkData benchmark_uint8x4(
    ...
    constexpr int tilesize = groupsize * numItems * 4;

    if constexpr(ApproachAndType::datatype != Datatype::UInt8x4){
    ...
    libmarv::GaplessAlignment_vec4score<21, ApproachAndType, blocksize, groupsize, numItems, isSingleTileBenchmark, !isSingleTileBenchmark> aligner;
    ...
    const int currentInt8SubstitutionScoreBias = std::abs(std::min(0, hostFullQueryPSSM.getMinElement()));
    //std::cerr << "currentInt8SubstitutionScoreBias " << currentInt8SubstitutionScoreBias << "\n";
    libmarv::PSSM fullPSSMWithBias(queryLength, alphabetSize);
    for(int r = 0; r < alphabetSize; r++){
        for(int c = 0; c < queryLength; c++){
            fullPSSMWithBias[r][c] = hostFullQueryPSSM[r][c] + currentInt8SubstitutionScoreBias;
        }
    }
    ...
    // libmarv::GpuPermutedPSSMforGapless gpuPermutedPSSMforGapless;
    // gpuPermutedPSSMforGapless.template fromGpuPSSMView<cuda::std::uint8_t, 16>(gpufullPSSMWithBias.makeView(), groupsize, numItems, currentInt8SubstitutionScoreBias, stream);
    ...
    try{
        ...
        for(int iteration = 0; iteration < timingIterations; iteration++){
            ...
            if(checkResults){
                ...
                for(int i = 0; i < numAlignments; i++){
                    float f; memcpy(&f, &h_scoreOutput[i], sizeof(float));
                    h_scoreOutput[i] = int(f);
                }
                int errors = 0;
                ...
                // }
                for(int i = 0; i < numAlignments; i++){
                    if(referenceScores[i] != h_scoreOutput[i] && h_scoreOutput[i] < 255 - currentInt8SubstitutionScoreBias){
                        errors++;
                        if(errors == 1){
...
struct ForEachCombination_call_singleconfigFunction{
    ...
    BenchmarkData helper2(
        ...
        if constexpr(ApproachAndType::datatype == Datatype::UInt8x4){
            const int queryLength = 4 * groupsize * numItems * tiles;
            if(queryLength > 2048){
                return makeNotExecutedBenchmarkData<blocksize, groupsize, numItems, 0, ApproachAndType>();
            }
            return benchmark_uint8x4<blocksize, groupsize, numItems, ApproachAndType, SleepDuration, singleTile>(
...
std::vector<BenchmarkData> benchmark_uint8x4_scoreOnly_multiTile(
    int timingIterations, 
    std::chrono::milliseconds sleepAmount,
    bool checkResults,
    int tiles
