#ifndef LIBMARV_SCORE_WITH_ENDPOS_ALIGNMENT_CUH
#define LIBMARV_SCORE_WITH_ENDPOS_ALIGNMENT_CUH
...
LIBMARV_NAMESPACE_WITH_NESTING_BEGIN
    ...
    >
    struct GaplessAlignment_endPos_scalarscore : public ScoreWithEndPosAlignmentInterface{
        public:
        int deviceId;
        ...
        GaplessAlignment_endPos_scalarscore(){
    ...
    template<
        ...
        bool enableMultiTile
    >
    struct GaplessAlignment_endPos_vec2score : public ScoreWithEndPosAlignmentInterface{
    public:
        int deviceId;
        int maxSharedMemoryPerBlockOptin;
        int numSMs;
        ...
        GaplessAlignment_endPos_vec2score(){
            CUDACHECK(cudaGetDevice(&deviceId));
            CUDACHECK(cudaDeviceGetAttribute(&maxSharedMemoryPerBlockOptin, cudaDevAttrMaxSharedMemoryPerBlockOptin, deviceId));
            CUDACHECK(cudaDeviceGetAttribute(&numSMs, cudaDevAttrMultiProcessorCount, deviceId));
        }
        ...
        void scoreEndPos_singleTile(
            ...
            if constexpr(!enableSingleTile){
                ...
                int smem = sizeof(SharedPSSM);
                auto kernel = [&](){
                    return hardcodedzero::GaplessFilter_strided_PSSM_endPos_singletile_kernel<
                        ScoreType,
                        blocksize, 
                        groupsize, 
                        numRegs, 
        ...
        void scoreEndPos_multiTile(
            ...
            if constexpr(!enableMultiTile){
                ...
                int smem = sizeof(SharedPSSM);
                auto kernel = [&](){
                    return hardcodedzero::GaplessFilter_strided_PSSM_endPos_multitile_kernel<ScoreType, blocksize, groupsize, 
                        numRegs, 
                        true, //withEndPosition
                        subjectIsCaseSensitive,
                        int*, PssmScoreType,
    ...
    template<
        ...
        bool enableMultiTile
    >
    struct GaplessAlignment_endPos_vec4score : public ScoreWithEndPosAlignmentInterface{
    public:
        int deviceId;
        int maxSharedMemoryPerBlockOptin;
        int numSMs;
        ...
        GaplessAlignment_endPos_vec4score(){
            CUDACHECK(cudaGetDevice(&deviceId));
            CUDACHECK(cudaDeviceGetAttribute(&maxSharedMemoryPerBlockOptin, cudaDevAttrMaxSharedMemoryPerBlockOptin, deviceId));
            CUDACHECK(cudaDeviceGetAttribute(&numSMs, cudaDevAttrMultiProcessorCount, deviceId));
        }
        ...
        void scoreEndPos_singleTile(
            ...
            if constexpr(!enableSingleTile){
                ...
                int smem = sizeof(SharedPSSM);
                auto kernel = [&](){
                    return uint8x4::GaplessFilter_strided_PSSM_singletile_uint8x4_kernel_new<
                        ScoreType,
                        blocksize, 
                        groupsize, 
                        numRegs, 
        ...
        void scoreEndPos_multiTile(
            ...
            if constexpr(!enableMultiTile){
                ...
                int smem = sizeof(SharedPSSM);
                auto kernel = [&](){
                    return uint8x4::GaplessFilter_strided_PSSM_multitile_uint8x4_kernel_new<ScoreType, blocksize, groupsize, 
                        numRegs, 
                        true, //withEndPosition
                        subjectIsCaseSensitive,
                        int*, PssmScoreType,
    ...
    template<
        ...
        bool enableMultiTile
    >
    struct SWAlignment_endPos_scalarscore : public ScoreWithEndPosAlignmentInterface{
    public:
        int deviceId;
        int maxSharedMemoryPerBlockOptin;
        int numSMs;
        ...
        static constexpr bool subjectIsCaseSensitive = true;
        static constexpr bool withEndPosition = true;

        SWAlignment_endPos_scalarscore(){
            CUDACHECK(cudaGetDevice(&deviceId));
            CUDACHECK(cudaDeviceGetAttribute(&maxSharedMemoryPerBlockOptin, cudaDevAttrMaxSharedMemoryPerBlockOptin, deviceId));
            CUDACHECK(cudaDeviceGetAttribute(&numSMs, cudaDevAttrMultiProcessorCount, deviceId));
        }
        ...
        void scoreEndPos_singleTile(
            ...
            if constexpr(!enableSingleTile){
                throw std::runtime_error("attempted to call scoreEndPos_singleTile, but singleTile was not enabled");
            }else{
        ...
        void scoreEndPos_multiTile(
            ...
            if constexpr(!enableMultiTile){
                throw std::runtime_error("attempted to call scoreEndPos_multiTile, but multiTile was not enabled");
            }else{
                if(strided_PSSM.groupsize != groupsize) throw std::runtime_error("pssm groupsize does not match aligner groupsize");
                if(strided_PSSM.numRegs != numRegs) throw std::runtime_error("pssm numRegs does not match aligner numRegs");
    ...
    struct MultiConfigScoreWithEndPosAligner : public MultiConfigScoreWithEndPosAlignerInterface{

        std::vector<std::unique_ptr<ScoreWithEndPosAlignmentInterface>> singleTileAlignerVector;
        std::vector<std::unique_ptr<ScoreWithEndPosAlignmentInterface>> multiTileAlignerVector;
        ...
        MultiConfigScoreWithEndPosAligner(AlignmentAlgorithmE algorithm_) : algorithm(algorithm_){
            ...
            auto addSingleTileAligner = [&](auto config, auto& vector){
                ...
                if constexpr(Config::algorithm == AlignmentAlgorithmE::Gapless){
                    ...
                    if constexpr(IsScalarType<typename ApproachAndType::ScoreType>::value){
                        using AlignerClass = GaplessAlignment_endPos_scalarscore<
                            Config::alphabetSize, 
                            ApproachAndType,
                        ...
                        using AlignerClass = GaplessAlignment_endPos_vec2score<
                            Config::alphabetSize, 
                            ApproachAndType,
                            Config::blocksize, Config::groupsize, Config::numRegs,
                            true,
                        ...
                        using AlignerClass = GaplessAlignment_endPos_vec4score<
                            Config::alphabetSize, 
                            ApproachAndType,
                            Config::blocksize, Config::groupsize, Config::numRegs,
                            true,
                    ...
                    using AlignerClass = SWAlignment_endPos_scalarscore<
                        Config::alphabetSize, 
                        ApproachAndType,
                        Config::blocksize, Config::groupsize, Config::numRegs,
                        true,
            ...
            if(singleTileAlignerVector.size() == 0 && multiTileAlignerVector.size() == 0){
                throw std::runtime_error("MultiConfigScoreWithEndPosAligner requires at least one config");
            }
        ...
        void scoreEndPos_singleTile(
            ...
            aligner->scoreEndPos_singleTile(
                devAlignmentScores,
                d_subjectEndPositionsExcl,
                d_queryEndPositionsExcl,
                inputData,
    ...
    private:
        ScoreWithEndPosAlignmentInterface* getBestSingleTileAligner(SequenceLengthT queryLength) const {
            if(singleTileAlignerVector.size() == 0){
                throw std::runtime_error("no single tile aligner available");
            }
            ScoreWithEndPosAlignmentInterface* bestAligner = std::lower_bound(
                singleTileAlignerVector.begin(),
                singleTileAlignerVector.end(),
                queryLength,
        ...
        ScoreWithEndPosAlignmentInterface* getBestMultiTileAligner(SequenceLengthT queryLength) const{
            if(multiTileAlignerVector.size() == 0){
                throw std::runtime_error("no multi tile aligner available");
            }

            ScoreWithEndPosAlignmentInterface* bestAligner = multiTileAlignerVector[0].get();
            ...
            if(performanceHintsMultiTile.size() == 0){
                ...
                for(size_t i = 1; i < multiTileAlignerVector.size(); i++){
                    ScoreWithEndPosAlignmentInterface* newAligner = multiTileAlignerVector[i].get();
        ...
        ScoreWithEndPosAlignmentInterface* getBestAligner(SequenceLengthT queryLength){
        ...
        const ScoreWithEndPosAlignmentInterface* getBestAligner(SequenceLengthT queryLength) const{
            if(isSingleTile(queryLength)){
                return getBestSingleTileAligner(queryLength);
            }else{                
                return getBestMultiTileAligner(queryLength);
...
#endif //LIBMARV_SCORE_WITH_ENDPOS_ALIGNMENT_CUH